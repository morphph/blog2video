分享XLinkedIn复制链接# Agent Loop：为什么最强 AI Agent 的核心只是一个 While 循环？
...
运行环境
终端原生，直接操作文件系统和 Shell
IDE 原生，在编辑器内运行

Agent Loop
完整的 while(true) + 无约束工具调用
有 Agent Mode，但受 IDE 插件架构约束

上下文窗口
最大 1M tokens
最大 200K tokens（可降低以加速）

工具执行
直接调用 bash、读写文件、操作 git
通过 IDE API 间接操作
...
### 相关内容#### 相关日报- 2026-04-07T00:00:00.000Z#### 相关术语- claude code- claude- what are claude code hooks- what is mcp claude code#### 相关对比- claude code remote vs ssh- claude code vs aider- claude code vs amazon q
