...
**Source Tweet:** https://x.com/coreyganim/status/2044137332920492467
**Linked Article:** https://x.com/i/article/2044133932354949121
**Primary Source:** https://claude.com/blog/introducing-routines-in-claude-code
...
Anthropic has launched routines in Claude Code research preview. Routines are Claude Code automations configured once with a prompt, repo, and connectors, then executed on a schedule, via API call, or triggered by events. They operate on Claude Code's web infrastructure, eliminating dependency on local machines.

## How It Works

### Scheduled Routines

Routines execute on defined cadences (hourly, nightly, or weekly):

"Every night at 2am: pull the top bug from Linear, attempt a fix, and open a draft PR."

Users familiar with `/schedule` in the CLI will find those tasks now become scheduled routines.

### API Routines

Routines accept HTTP POST requests with unique endpoints and authentication tokens. Each routine returns a session URL:

"Read the alert payload, find the owning service, and post a triage summary to #oncall with a proposed first step."

This enables integration with alerting systems, deployment hooks, and internal tools.

### Webhook Routines with GitHub

Routines subscribe to GitHub repository events, creating new sessions for matching PRs. Claude processes updates including comments and CI failures:

"Flag PRs that touch the /auth-provider module. Any changes need summarization and posting to #auth-changes."

Future expansions will support additional event sources.

---

# Full Documentation: Automate work with routines
