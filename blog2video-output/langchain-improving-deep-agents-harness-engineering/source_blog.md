...
[IMAGE DESCRIPTION: Terminal Bench 2.0 leaderboard screenshot showing top 10 coding agents. #1 Simple Codex (GPT-5.3-Codex, OpenAI) 75.1%, #2 CodeBrain-1 (GPT-5.3-Codex, Feeling AI) 70.3%, #3 Droid (Claude Opus 4.6, Factory) 69.9%, #4 Mux (GPT-5.3-Codex, Coder) 68.5%, #5 Deep Agents (GPT-5.2-Codex, LangChain) 66.5% highlighted with red border, #6 Droid (GPT-5.2, Factory) 64.9%, #7 Ante (Gemini 3 Pro, Antigma Labs) 64.7%, #8 Terminus 2 (GPT-5.3-Codex, Terminal Bench) 64.7%, #9 Junie CLI (Gemini 3 Flash, JetBrains) 64.3%, #10 Droid (Claude Opus 4.5, Factory) 63.1%]
...
[IMAGE DESCRIPTION: Progress table showing 3 experiment runs. Baseline (baseline prompt for coding, file system tools, planning) scored 52.8%. +Custom Prompt & Middleware (focus on Build-Verify Loop, inject environment context, add loop protection and timeout warnings) scored 63.6%. +Adaptive reasoning (xhigh+high reasoning) scored 66.5%.]
...
[IMAGE DESCRIPTION: Trace Analyzer Skill diagram showing 3 stages. Step 1 FETCH: LangSmith Tracing Project fetches all N Traces (raw data). Step 2 ANALYZE: traces split into batches (Batch 1, 2, 3) each processed by parallel Sub-Agents for error analysis, results flow into Main Agent that synthesizes findings. Step 3 REVIEW (Human in the Loop): findings + suggestions for next experiment go to Human Review who approves changes for next experiment.]
...
[IMAGE DESCRIPTION: Agent Self-Verification within a Run diagram showing a loop: Build A Solution → Verify Against Spec & Run Tests → Refine from Errors → back to Build A Solution. Arrow labeled "iterate until correct".]
...
2. **Teaching Agents to Write Testable Code:** Agents don't know how their code needs to be testable. We add prompting saying their work will be measured against programatic tests, similar to when committing code. For example, Task specs that mention file paths should be followed exactly so the solutions work in an automated scoring step. Prompting that stresses edge-cases helps the agent avoid only checking "happy path" cases. Forcing models to conform to testing standards is a powerful strategy to avoid "slop buildup" over time.
...
[IMAGE DESCRIPTION: The Reasoning Sandwich diagram showing 3 phases on a time budget timeline: Phase 1 "xhigh reasoning" for Plan & Understand (first 25% of budget), Phase 2 "high reasoning" for Build & Iterate (middle 50% — fast), Phase 3 "xhigh reasoning" for Final Verification (last 25% of budget).]
