...
1. SKILL.md Automatically generate a draft of

1. Test it by actually running it with the test prompt

1. Evaluate the results and propose improvements

1. Repeat steps 2 to 4 until you are satisfied

The idea is that Claude Code itself will go through the cycle of manual SKILL.mdwriting, trying, and fixing things.
...
Skills are actually a form of plugin. We use anthropics/skills marketplaceInstall Skills through plugins in the marketplace, and Claude will automatically load them when needed.
...
After installation, we’re going to check if the /skill-creator available, I am going to ask the Claude Claude code:
