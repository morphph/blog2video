...
_Thariq (@trq212) · Jul 3, 2026_

![Image](images/image_1.jpg)
<!-- [IMAGE DESCRIPTION] 标题头图（深色底）。左侧标题「A Field Guide to Fable: Finding Your Unknowns」。右侧一张风格化插画：一块白色有机形状（象征"territory"地形），一条蜿蜒路径从起点圆点向上攀升、穿过节点和几座小山（三角形），末端一条虚线拖向一个问号——可视化从"地图"走向"地形"、以及地形之外的未知。 -->
...
<!-- [IMAGE DESCRIPTION] 核心「地图 vs 地形」概念图。左格「The map」：干净的网格上，一条笔直的虚线对角线从红色起点连到目标圆圈（理想化的计划）。右格「The territory」：同样的起终点，但真实路径绕开一堆障碍——一座山、几个问号、地形等高线。两格之间一个双向箭头，标注「your unknowns」——计划与现实之间的落差。 -->
...
To do this, you can ask Claude to help you find your unknown unknowns and explain them to you. I like to use the literal words "blindspot pass" and "unknown unknowns". Giving it context on who you are and what you know is usually important for it.
...
<!-- [IMAGE DESCRIPTION] 2×2「未知」四象限框架卡片。四张卡：Known knowns「what's in your prompt / 你告诉 agent 你想要什么」（文档图标）；Known unknowns「questions you know to ask / 你还没弄清、但你知道自己没弄清」（虚线框+问号）；Unknown knowns「'I'll know it when I see it' / 太显然以至于你不会写下来，但看到就认得」（眼睛图标）；Unknown unknowns「what you never considered / 你没料到这条路上会有的坑」（深色卡+红色问号）。 -->
...
<!-- [IMAGE DESCRIPTION] 冰山 before/after 隐喻。「before」：冰山水面上只露一小角，水面下庞大部分塞满问号（隐藏的未知）。中间箭头「finding your unknowns」→「after」：更多冰山浮出水面、露出的部分带上了可见的文字线条（未知被浮现/写明），水下问号变少。 -->
...
<!-- [IMAGE DESCRIPTION] 全套技法的纵向时间线总图。「Before implementation」：Blind spot pass、Brainstorms & prototypes、Interviews、References、Implementation plan。「During implementation」：Implementation notes（log deviations, keep going）。「After implementation」：Pitches & explainers、Quizzes（merge only when you pass）。一条虚线反馈回路从底部绕回顶部；底部标语「what you learn becomes the map for next time」。 -->
...
<!-- [IMAGE DESCRIPTION]「Pitches & explainers」图。prototype + spec + notes 三个盒子汇合成「one doc」（内含一个演示视频播放器，标注"lead with the demo"）→ 箭头指向两个带绿色对勾的审阅者人像 =「buy-in」。底部标语「reviewers start with the same unknowns you did — answer them up front」。 -->
