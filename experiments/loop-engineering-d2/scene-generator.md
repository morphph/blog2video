# scene-generator —— d2「终端霓影」单场景生成器(智能模板)

> 给**子 agent** 的完整作业书。一个 agent 负责一个(或一小批)场景:把旧暗色版 `slide_N.html` 的内容,
> 在 d2 视觉语言里重排成一个**自包含、可单独渲染的静音场景**,字幕烧入,动效绑口播。
> 子 agent 无共享上下文 —— 本文件 + kit + 同类型黄金范本 = 你需要的全部。
...
| 输入 | 路径 |
|---|---|
| 本场景 brief(时长/字幕本地时间/topbar 序号/源 slide 路径) | `briefs/scene-NN.json` |
| 旧暗色版源 slide(内容真源:标题/卡片/数字/命令/术语) | `blog2video-output/loop-engineering/slide_N.html`(= brief.srcSlideHtml) |
...
| 你的类型 | 范本 src | 范本说明 |
|---|---|---|
| cover | `scenes/scene-01/index.html` | 多 beat 封面(已渲已验收) |
| image | `src/scene-02.html` | kicker+标题 + 编号步骤清单(逐条 build,行内 code/cyan 标签) |
| principle | `src/scene-05.html` | 第一性原理 + corner-bracket 概念 + 时间线逐格点亮(末格 .now+badge) |
| comparison_cards | `src/scene-07.html` | 双栏(左 acid / 右 .alt cyan)逐栏 build + 底部 status |
| quote | `src/scene-16.html` | 终端卡金句(coral 强调)+ stats count-up + win 大数字 |
| checklist | `src/scene-17.html` | 4 原则编号清单(密集:画布只放关键词,长例子留字幕)+ warn 行 |
| summary | `src/scene-24.html` | 收官(brand 改"收官")+ 终端命令 CTA + 酸性 sweep + endchip |

## 2. 输出契约

1. 写 **`src/scene-NN.html`** —— 完整 HTML 文档,但 kit 用 marker 占位:
   - `<style>` 内放 `/*@@BASE@@*/`(build 时替换成 d2-base.css 全文)+ 极少量本场景特有样式
   - 第一个 `<script>` 放 `/*@@MOTION@@*/`(build 时替换成 d2-motion.js 全文)
   - 第二个 `<script>` 写本场景 timeline
2. `node build.mjs NN` → 生成自包含 `scenes/scene-NN/index.html`(场景目录已由 `./scaffold-scene.sh NN` 建好:assets symlink + hyperframes.json + meta.json)。
...
1. **时长精确**:根 `data-duration` = `brief.dataDuration`,一秒不差(concat 靠它对齐)。
...
3. **字幕烧入**:`brief.subtitles` 每条一个 `<div class="clip sub" data-start data-duration data-track-index="5">`,
   **用本地时间(brief 里的 localStart/localDuration)**,文字**一字不差**照 brief。每条 `<div class="sub-inner">` 里**最多 1 个 `.sub-k`**(酸性绿)点关键词。入场 0.2s 淡入。
   (build.mjs 会自动把每条显示时长削 10ms 避免 track5 浮点重叠 —— 你照 brief 原值写即可。)
...
8. **字体**:只用 MiSans + JetBrains Mono(已在 assets/fonts)。**font-family 写字面量** `'MiSans',sans-serif` / `'JetBrains Mono',monospace`,**不要写 `var(--sans)`/`var(--mono)`**(lint 会误报字体缺失)。**禁止引入新大字体**。
...
- **cta**(s25,5s 静音尾卡,**无字幕**):纯 end card —— 大 logo `精读AI`(MiSans 900)+ 副标 `LOOP ENGINEERING · 完`(mono)+ 酸性 `.cur` 光标(blink)。无 topbar 页码亦可。
...
cd /Users/yufanp/Desktop/Project/blog2video/experiments/loop-engineering-d2
node build.mjs NN
hyperframes lint scenes/scene-NN 2>&1 | grep -E "✗|error\(s\)"    # 必须 0 error
NODE_OPTIONS="--max-old-space-size=5120" hyperframes snapshot scenes/scene-NN --at <场景内 5-6 个关键时刻,本地秒> --describe false
```
然后用 **Read 工具看 `scenes/scene-NN/snapshots/contact-sheet.jpg`**,逐格检查:
...
每个场景报:文件路径、lint 结果(error 数)、快照目检发现并修复的问题清单、与 srcSlide 的内容差异(应只换皮不改数据 + 删 emoji)、用到的辅助色。**不要自己 render mp4**(渲染由编排者统一串行做)。
